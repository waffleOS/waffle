...
bool inode_extend(struct inode *inode, off_t offset) {
    ...
    if (offset >= length)
    {
        ...
        cache_sector *sector_ind = cache_write_sector(disk_inode->indirect); 
        struct indirect_inode_disk * indirect_data = (struct indirect_inode_disk *) sector_ind->data;
        ...
        if (last_block < NUM_DIRECT + NUM_ENTRIES + NUM_ENTRIES * NUM_ENTRIES)
        {
            ...
            cache_sector *indirect_sector = cache_write_sector(disk_inode->double_indirect);
            struct indirect_inode_disk * double_indirect_data = (struct indirect_inode_disk *) indirect_sector->data;
            while (indirect_index < NUM_ENTRIES)
            {
                sector = cache_write_sector(double_indirect_data->sectors[indirect_index]);
                struct indirect_inode_disk * indirect_data = (struct indirect_inode_disk *) sector->data;
...
*/
    ...
    cache_sector *inode_sector = cache_write_sector(inode->sector);
    struct inode_disk * disk_inode = (struct inode_disk *) inode_sector->data;
    ...
    /* Done writing inode_sector */
    done_write(&inode_sector->rw);
